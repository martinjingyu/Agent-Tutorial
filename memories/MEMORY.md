...
§
Round 4 revised position: Adopting aggregated plan's R4 edge-density+colorfulness rule, DBSCAN diversity clustering (with my tiered as fallback), scene-tier allocation with quotas, and circular round-robin selection. Keeping my own additions: KL-divergence bias check for truro_school, nearest-neighbor score propagation for reduced Stage B mode, face-detection reliability guard for anti-portrait penalty. Fully aligned with aggregated plan on all core decisions.
